...
RequestBuilder.prototype.invoke = function (parameters, cb) {
  ...
  if (req.json === undefined) {
    for (const h in req.headers) {
      if (h.toLowerCase() === "accept" || h.toLowerCase() === "content-type") {
        req.json = req.headers[h] === "application/json";
      }
    }
  }
  ...
  if (typeof json.options === "object") {
    for (const o in json.options) {
      req[o] = req[o] ?? json.options[o];
    }
  ...
  if (cb) {
    callback = (err, response, body) => {
      ...
      let result = body;
      if (jsonPath && this.template.template.responsePath) {
        result = jsonPath({
          wrap: true,
          json: body,
          path: this.template.template.responsePath,
        });
      }
      if (this.template.template.fullResponse) {
        result = response;
      }
      if (debug.enabled) {
 ...
 */
RequestBuilder.prototype._request = function (uri, options, callback) {
  if (typeof uri === "undefined") {
    const errMsg = g.f(
      "{{undefined}} is not a valid {{uri}} or {{options}}" +
        " object.",
    );
    throw new Error(errMsg);
  }
  callback = ((typeof options === "function") && !callback)
    ? options
    : callback;
  ...
  const context = { request: this.request, req: options };
  const work = (context, done) => {
    this.request(options, function (err, res, body) {
      context.res = res;
      done(err, res, body);
    });
  };
  this.connector.notifyObserversAround("execute", context, work, callback);
};
